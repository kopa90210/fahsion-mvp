...
export default function PhotoUploadForm({ onUpload, submitLabel = 'Upload photo' }: { onUpload: (file: File) => Promise<unknown>; submitLabel?: string }) {
  ...
  return <form onSubmit={submit} className="space-y-4">
    <label className="flex min-h-48 cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed border-[#b9aa99] bg-white/45 p-6 text-center">
      ...
      <span className="mt-1 text-xs text-[#7a6f62]">Use a clear photo of the item.</span>
      <input type="file" accept="image/*" capture="environment" className="sr-only" onChange={(event) => setFile(event.target.files?.[0] ?? null)} />
    </label>
