...
import { confirmDraftItem, discardDraftItem, removeWardrobeItem, type UserWardrobeItem } from '@/src/app/actions/wardrobe'
import { SUBCATEGORY_MAP, type WardrobeCategory } from '@/src/lib/wardrobe/normalize'

function ItemCard({ item, selected, selecting, onToggle }: { item: UserWardrobeItem; selected: boolean; selecting: boolean; onToggle: () => void }) {
  const label = item.display_name || item.subcategory || 'Wardrobe item'
  return <button type="button" onClick={onToggle} className={`relative min-w-0 text-left ${selecting ? 'cursor-pointer' : 'cursor-default'}`}>
...
export default function WardrobeScreen({ items, drafts, categories }: { items: UserWardrobeItem[]; drafts: UserWardrobeItem[]; categories: readonly WardrobeCategory[] }) {
  ...
  return <main className="min-h-screen bg-[#f7f4ef] px-4 py-7 text-[#1d1b18] sm:px-6 lg:py-10"><div className="mx-auto max-w-5xl">
    ...
    {showDrafts ? <section className="mt-8"><div className="mb-4 flex items-center justify-between"><h2 className="text-xl font-semibold">Drafts</h2><button type="button" onClick={() => setShowDrafts(false)} aria-label="Close drafts"><X className="size-5" /></button></div><div className="grid grid-cols-3 gap-x-3 gap-y-6">{drafts.map((item) => <div key={item.id}><ItemCard item={item} selected={false} selecting={false} onToggle={() => undefined} /><div className="mt-2 flex gap-2"><Button size="sm" onClick={() => draftAction(() => confirmDraftItem(item.id), 'Draft confirmed.')} disabled={isPending}>Confirm</Button><Button size="sm" variant="outline" onClick={() => draftAction(() => discardDraftItem(item.id), 'Draft discarded.')} disabled={isPending}>Discard</Button></div></div>)}</div></section> : <div className="mt-8 grid grid-cols-3 gap-x-3 gap-y-6">{visible.map((item) => <ItemCard key={item.id} item={item} selected={selected.includes(item.id)} selecting={selecting} onToggle={() => toggle(item.id)} />)}</div>}
